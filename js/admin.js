...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        newOrderForm.addEventListener('submit', (e) => {
            ...
            createNewOrder(nomeCliente, endereco, nomeBolo, cep, rua, bairro, numero, complemento, whatsapp);
            newOrderForm.reset();
            newOrderModal.style.display = 'none';
        });
        ...
        const newOrderModalCloseBtn = newOrderModal.querySelector('.close-button');
    ...
    function listenToFirebase() {
        ...
        onValue(locationRef, (snapshot) => {
            alexandreLocation = snapshot.val();
     ...
     */
    function createNewOrder(nomeCliente, endereco, nomeBolo, cep, rua, bairro, numero, complemento, whatsapp) {
        ...
        update(ref(db, 'pedidos'), updates).catch(err => console.error("Erro ao criar pedido:", err));
    }
    ...
    function createOrderCard(pedidoId, pedido) {
        ...
        card.innerHTML = `<h4>${pedido.nomeBolo || 'Bolo'}</h4><p>${pedido.nomeCliente}</p><p>${pedido.endereco}</p><div class="distance"></div>`;
    ...
    function updateDeliveryMarker() {
        if (alexandreLocation && map) {
            const { latitude, longitude } = alexandreLocation;
            ...
            deliveryMarker.bindPopup("Localização do Alexandre").openPopup();
            map.panTo(latLng);
        }
    }
    ...
    async function geocodeAddress(address) {
        ...
        try {
            const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(addressForQuery)}`);
            const data = await response.json();
    ...
    async function findAndHighlightClosest() {
        if (!alexandreLocation) return;

        const readyOrdersColumn = kanbanBoard.querySelector('.kanban-column[data-status="pronto_para_entrega"]');
        ...
        for (const card of orderCards) {
            ...
            if (pedidoSnapshot.exists()) {
                ...
                if (pedidoCoords) {
                    const dist = calcularDistancia(alexandreLocation.latitude, alexandreLocation.longitude, pedidoCoords.lat, pedidoCoords.lon);
                    distanceSpan.textContent = `${dist.toFixed(1)} km`;
                    if (dist < closestOrder.distance) {
                        closestOrder = { id: pedidoId, distance: dist, clientName: pedido.nomeCliente };
                    }
                } else {
        ...
        if (closestOrder.id) {
            ...
            if (closestCard) {
                ...
                columnTitle.textContent = `Próximo: ${closestOrder.clientName} (${closestOrder.distance.toFixed(1)} km)`;
            }
        } else {
    ...
    function extractDataFromMessage() {
        ...
        if (!extractedData.clientName || !extractedData.cakeName || !extractedData.rua) {
            alert('Não foi possível extrair os dados do pedido. Verifique se a mensagem está no formato correto.');
            return;
        }

        // Preenche os campos do formulário de novo pedido
        for (const key in fields) {
            if (Object.prototype.hasOwnProperty.call(fields, key) && extractedData[key]) {
